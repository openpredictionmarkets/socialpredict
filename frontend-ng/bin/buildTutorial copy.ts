...
const outPath = path.resolve(__dirname, '../src/lib/components/TutorialModal2.svelte');
...
svelte += `    backdrop-filter: blur(6px);\n`;
...
svelte += `    background:\n`;
svelte += `      radial-gradient(120% 120% at 0% 0%, var(--color-primary-soft, rgba(159, 107, 255, 0.15)), transparent 45%),\n`;
svelte += `      radial-gradient(140% 140% at 100% 0%, var(--color-accent-soft, rgba(53, 226, 209, 0.12)), transparent 40%),\n`;
svelte += `      var(--panel, #12131a);\n`;
