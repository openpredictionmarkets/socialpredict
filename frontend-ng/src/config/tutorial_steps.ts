type TutorialStepDescriptionItem = {
  key: string;
  label: string;
  detail: string;
};
...
export type TutorialStep = {
  title: string;
  description: string | TutorialStepDescriptionItem[];
  note?: string;
};
...
export const defaultTutorialSteps: TutorialStep[] = [
  {
    ...
    `,
    note: '<p>Tip: Share prices represent outcome probabilities. For example, if the market price for YES is 62¢, the market expects that there is a ~62% chance it will resolve YES.</p>'
  },
  {
    title: 'Tour a market…',
    description: [
  ...
  },
  {
    ...
    `,
    note: 'Press the "Resolve Market to YES" button to see how your payout is calculated based on your purchase price versus the resolution outcome.'
  },
  {
