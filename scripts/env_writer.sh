...
sed -i -e "s/domain.com/$domain_answer/g" .env
...
sed -i -e "s/user@example.com/$email_answer/g" .env
...
if [ ! -z "$db_user_answer" ]; then
	# Change DB User:
	sed -i -e "s/user/$db_user_answer/g" .env
...
if [ ! -z "$db_pass_answer" ]; then
	# Change DB Password:
	sed -i -e "s/password/$db_pass_answer/g" .env
...
if [ ! -z "$db_name_answer" ]; then
	# Change DB Name:
	sed -i -e "s/socialpredict_db/$db_name_answer/g" .env
...
if [ ! -z "$admin_pass_answer" ]; then
	...
	sed -i -e "s/adminpass/$admin_pass_answer/g" .env
