...
| --- | --- | --- | --- |
| Legacy markets reads and writes | The market read-route and action failure slices now use shared failure envelopes; remaining raw `http.Error` behavior is limited to the disabled create compatibility bridge. Some successful delete/resolve paths still intentionally write status-only `204` responses. | `createmarket.go` compatibility bridge | Retire the remaining compatibility entry point without changing successful status-only action semantics. |
