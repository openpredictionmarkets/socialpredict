...
| `ADMIN_PASSWORD` | Admin login password generated during staging install |

> The admin password is regenerated by the current install/deploy flow. Use `./HostOps host env get staging ADMIN_PASSWORD` after deployment if operator access is configured.
...
✅ Admin password can be retrieved with HostOps after deployment
